...
public class MetadataLookup {
	...
	public static Program getProgram(String lookup) {
		...
		if (program == null && StringUtils.isNotBlank(lookup)) {
	...
	public static ProgramWorkflow getProgramWorkflow(String programLookup, String workflowLookup) {
		...
		ProgramWorkflow wf = p != null ? p.getWorkflowByName(workflowLookup) : null;

		if (wf == null && p != null) {
			for (ProgramWorkflow programWorkflow : p.getAllWorkflows()) {
				if (workflowLookup.equalsIgnoreCase(programWorkflow.getConcept().getName().getName())) {
					wf = programWorkflow;
		...
		if (wf == null && StringUtils.isNotBlank(programLookup) && StringUtils.isNotBlank(workflowLookup)) {
	...
	public static ProgramWorkflowState getProgramWorkflowState(String programLookup, String workflowLookup,
		...
		ProgramWorkflowState s = wf != null ? wf.getStateByName(stateLookup) : null;
		if (s == null && wf != null) {
			for (ProgramWorkflowState state : wf.getStates()) {
				if (stateLookup.equalsIgnoreCase(state.getConcept().getName().getName())) {
					s = state;
		...
		if (s == null && StringUtils.isNotBlank(stateLookup) && StringUtils.isNotBlank(programLookup)
				&& StringUtils.isNotBlank(workflowLookup)) {
			throw new IllegalArgumentException(
	...
	public static Concept getConcept(String lookup) {
		...
		if (c == null && StringUtils.isNotBlank(lookup)) {
	...
	public static Form getForm(String lookup) {
		...
		if (form == null && StringUtils.isNotBlank(lookup)) {
	...
	public static EncounterType getEncounterType(String lookup) {
		...
		if (et == null && StringUtils.isNotBlank(lookup)) {
	...
	public static RelationshipType getRelationshipType(String lookup) {
		...
		if (rt == null && StringUtils.isNotBlank(lookup)) {
	...
	/**
	 * @return the OrderType that matches the passed uuid, name, or primary key
	 *         id
	 */
	public static OrderType getOrderType(String lookup) {
		...
		if (ot == null && StringUtils.isNotBlank(lookup)) {
