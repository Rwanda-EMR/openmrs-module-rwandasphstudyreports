...
public class GlobalPropertiesManagement {
	...
	public ProgramWorkflowState getProgramWorkflowState(String globalPropertyName, String workflowName,
			String programName) {
		String programGp = Context.getAdministrationService().getGlobalProperty(programName);
	...
	public Map<Concept, Double> getVialSizes() {
		...
		for (String vial : vials) {
			...
			try {
				...
				throw new IllegalArgumentException("Unable to convert " + vial + " into a vial size Concept and Double",
						e);
			}
		}
		return vialSizes;
